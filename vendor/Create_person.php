...
try {
    ...
    $kcal = round(GetKkal($sex, $activity, $weight, $height, $age), 2);
    $carb = round(GetCarb($kcal), 2);
    $fat = round(GetFat($kcal), 2);
    $protein = round(GetProtein($kcal), 2);
    $cellulose = round(GetCellulose($kcal), 2);
...
//от калорийности суточного рациона для омега-3.
    $om3 = round(0.06 * $kcal, 2);
    $om6 = round(0.015 * $kcal, 2);
    $vitB1 = 1.3;
    $vitB2 = 1.5;
    $vitB5 = 4;
    $vitB6 = 1.6;
...
mysqli_query($soe, "INSERT INTO `persons`(`ID`, `Name`, `Sex`, `age`, `Date_of_birth`, `activity`, `Weight`, `Height`, `kkal`, `carb`, `fat`, `protein`, `cellulose`, `water`, `vitA`, `vitE`, `vitK`, `vitD`, `vitC`, `om3`, `om6`, `vitB1`, `vitB2`, `vitB5`, `vitB6`, `vitB8`, `vitB9`, `vitB12`, `minMg`, `minNa`, `minCl`, `minCa`, `minK`, `minS`, `minP`, `minCu`, `minI`, `minCr`) 
 VALUES ('$name','$sex','$age','$date','$activity','$weight','$height',
 '$kcal','$carb','$fat','$protein','$cellulose','$water' ,
